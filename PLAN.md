...
- **Implemented:** workspace/package polish for crates.io has started: internal workspace dependencies now carry publishable versions, shared manifest metadata is inherited from the workspace, the local NVENC binding crate is routed through a `[patch.crates-io]` override, the publishable crates now point at shared docs.rs/readme metadata, and the demo/probe binaries are marked `publish = false`
...
- **Next up:** finish the remaining crates.io polish — current dry-runs indicate the release order should be `ustreamer-proto` -> `ustreamer-capture` -> (`ustreamer-input`, `ustreamer-quality`, `ustreamer-transport`, `ustreamer-encode`) -> `ustreamer-app` — then validate the new GStreamer fallback on real Linux/Windows plugin stacks
...
- **publish-readiness**: workspace manifests now inherit shared license/repository/homepage metadata, internal workspace dependencies carry publishable versions, the vendored NVENC binding crate is redirected through a local `[patch.crates-io]`, the publishable crates now carry docs.rs/readme/keyword metadata, and `ustreamer-demo` / `ustreamer-nvenc-probe` are marked `publish = false`. Remaining work is validating publish order and dry-runs for the publishable crates once the Phase 3 backend work lands
