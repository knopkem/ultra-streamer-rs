...
- **Implemented:** workspace/package polish for crates.io has started: internal workspace dependencies now carry publishable versions, shared manifest metadata is inherited from the workspace, the local NVENC binding crate is routed through a `[patch.crates-io]` override, and the demo/probe binaries are marked `publish = false`
- **Next up:** stand up the Phase 3 GStreamer fallback backend for AMD/unsupported GPUs and then finish the remaining crates.io polish (publish order, dry-runs, and any crate-level metadata/docs adjustments still needed)
...
- **publish-readiness**: workspace manifests now inherit shared license/repository/homepage metadata, internal workspace dependencies carry publishable versions, the vendored NVENC binding crate is redirected through a local `[patch.crates-io]`, and `ustreamer-demo` / `ustreamer-nvenc-probe` are marked `publish = false`. Remaining work is docs.rs/publish metadata polish where needed plus validating publish order and dry-runs for the publishable crates
