...
- **Next up:** validate the new exported timeline semaphore path on real NVIDIA hardware, replace the conservative host-side wait with a true GPU-driven handoff, then wire NVENC session/bitstream output and backend-specific true-lossless refine where supported
