...
- **Next up:** re-run the full browser demo path on the Windows RTX 2070 host to confirm both the CUDA-array interop fix and the new HEVC lossless-refine path while verifying the new sync-mode auto-detect cleanly stays on `HostSynchronized` there, then replace the conservative host-side wait with a true GPU-driven handoff and validate end-to-end AV1 on AV1-capable NVIDIA hardware
