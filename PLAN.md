...
- **Implemented:** the direct NVENC HEVC path now queries sequence parameters from NVENC, builds browser-ready `hvcC` decoder config, and normalizes HEVC access units into the same length-prefixed format already consumed by the WebCodecs client; the AV1 path now extracts a WebCodecs-ready Sequence Header OBU and derives an RFC 6381 codec string from it
