...
- **Next up:** re-run the full browser demo path on the Windows RTX 2070 host after the CUDA-array interop fix, then replace the conservative host-side wait with a true GPU-driven handoff, add AV1 decoder-config extraction, and wire backend-specific true-lossless refine where supported
...
- **encoder-nvenc-direct**: feature-gated `NvencEncoder` now opens a real NVENC session, registers imported CUDA arrays, returns real bitstreams, queries HEVC sequence parameters into browser-ready `hvcC`, and normalizes HEVC access units into the length-prefixed format already consumed by the browser client. Real Windows hardware now confirms the host-sync CUDA import and HEVC bitstream path is alive, and the distorted browser output fix has switched optimal-tiled Vulkan image imports from linear CUDA buffers to CUDA mipmapped arrays / `CUDAARRAY` registration. Remaining work is consuming future GPU sync handles, AV1 decoder-config extraction, backend-specific true-lossless refine, and final runtime confirmation on the Windows demo host
