...
- **Implemented:** `ustreamer-demo` now probes the local platform/GPU/encoder capabilities it can see at startup and automatically chooses the best backend/codec/sync mode for the current machine — currently VideoToolbox HEVC on macOS and, for `nvenc-direct`, AV1 when the selected/default CUDA device reports support otherwise HEVC plus exported timeline semaphore only when the active Vulkan device exposes external-semaphore export (falling back to `HostSynchronized` otherwise and again on timeline-path runtime faults) — while keeping explicit CLI overrides for validation (`--codec`, device selection, future force-backend flags)
...
- **In progress:** direct AMF runtime validation on real Windows/Linux AMD systems has already flushed out two driver-compatibility fixes in the bring-up path — the probe now uses encoder-safe dimensions instead of `64x64`, and HEVC nominal-range configuration now uses `HevcNominalRange` instead of the less portable `HevcInputFullRangeColor`. Remaining work is end-to-end browser playback validation plus any AMD-specific tuning discovered there; after that, expand AV1 runtime validation on AV1-capable hardware and execute the staged crates.io publish when desired
...
- **lossless-settle** *(done)*: Idle detection → forced refine keyframe, explicit `refine` vs `lossless` signaling on the wire; VideoToolbox remains visually-lossless only, while direct NVENC HEVC can now emit true lossless refine frames on supported devices
...
- **encoder-amf-direct** *(implemented, hardware validation in progress)*: a feature-gated direct AMF HEVC backend now exists in `ustreamer-encode` for AMD/unsupported GPU hosts — staged `CapturedFrame::CpuBuffer` input, runtime loading from the installed AMD driver, HEVC AU normalization + decoder-config extraction for the existing browser path, and demo integration behind `amf-direct`. Ongoing AMD bring-up has already fixed probe-size compatibility (`128x128+` instead of `64x64`) and switched HEVC full-range signaling to the more driver-tolerant `HevcNominalRange` property. Remaining work is real Linux/Windows end-to-end validation plus any backend-specific tuning discovered there
